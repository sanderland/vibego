...
\cite{alphazero2018}. Every engine we discuss is a variation on this template; the
...
known game tree'' to ``search a learned model.'' Subsequent work makes MuZero cheaper to
train (\textbf{ReZero} reuses buffer information via backward-view reanalysis
\cite{rezero2024}) and more interpretable (work demystifying what its learned latent model
actually represents during planning \cite{demystifymuzero2024}).
...
\subsection{Residual trunks and the road to ``nbt''}
\label{sec:nbt}
...
are long-range. Several works study this directly. \textbf{Vision Transformers for
...
\cite{chessformer2024} trains an encoder-only transformer via AlphaZero-style self-play.
...
Table~\ref{tab:engines} summarizes the principal engines of the deep-learning era.
...
\label{sec:emerging}
Three threads are worth watching. First, \textbf{language models as planners}: recent work
...
chess-playing networks \cite{interpchess2024}, complementing the Othello world-model results
and bearing directly on \emph{why} the cyclic blind spot exists. Third, \textbf{architecture
search} continues, with the transformer-vs-convolution question (Section~\ref{sec:arch}) far
from settled --- the strongest current evidence is that hybrids help on long-range patterns
\cite{restnet2025} while representation can matter more than the backbone \cite{alphavile2024}.
...
play becomes, and an adversary trained explicitly to seek them out keeps finding the gap.
...
\bibitem{minizero2024} T.~Wu \emph{et al.},
  ...
  games,'' \emph{IEEE Transactions on Games}, 2024. arXiv:2310.11305.
...
\bibitem{chessformer2024} D.~Monroe and The Leela Chess Zero Team,
...
\bibitem{sai2019} F.~Morandin, G.~Amato, M.~Gini, C.~Metta, M.~Parton, G.-C.~Pascutto,
...
\bibitem{lndsolver2025} C.-C.~Shih \emph{et al.},
  ...
  \emph{IEEE Transactions on Games}, 2025. arXiv:2512.21365.
